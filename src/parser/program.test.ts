...
describe("declarations", () => {
  test("base unit", () => {
    expect(show("declare kg base mass")).toBe("(declare kg (base mass))")
  })

  test("composite unit (body is an Expr, not a unit_expr)", () => {
    expect(show("declare gr (kg / 1_000)")).toBe(
      "(declare gr (composite (div kg 1000)))",
    )
  })

  test("alias (dimension-only, for dynamic-rate currencies)", () => {
    expect(show("declare kzt currency")).toBe("(declare kzt (alias currency))")
  })

  test("composite with multi-token expression", () => {
    expect(show("declare rub (usd / 90,5)")).toBe(
      "(declare rub (composite (div usd 90.5)))",
    )
  })

  test("malformed declare reports a diagnostic", () => {
    const { diagnostics } = parse("declare")
    expect(diagnostics.length).toBeGreaterThan(0)
  })

  test("declare expecting unit_def body", () => {
    const { diagnostics } = parse("declare kg")
    expect(diagnostics.length).toBeGreaterThan(0)
  })
})
...
describe("variable declarations", () => {
  ...
  test("with simple unit (unit must be declared first)", () => {
    expect(show("declare kg base mass\n35,5 kg flour")).toBe(
  ...
  test("simple unit not declared yields a diagnostic", () => {
    ...
    expect(diagnostics[0]?.hint).toContain("declare foo")
...
describe("expression assignments", () => {
  ...
  test("with conversion via 'as'", () => {
    expect(show("declare kzt currency\nsalary as kzt = salaryInKzt")).toBe(
      "(declare kzt (alias currency))\n(= salaryInKzt (as salary kzt))",
    )
  })
})
...
describe("bare expressions", () => {
  ...
  test("quantity literals (right operand wins later in eval)", () => {
    const src = ["declare usd base currency", "declare rub currency", "100 rub + 5 usd"].join("\n")
    expect(show(src)).toBe(
      [
        "(declare usd (base currency))",
        "(declare rub (alias currency))",
...
describe("unit expressions", () => {
  test("simple ref", () => {
    expect(show("declare gr (kg)")).toBe("(declare gr (composite kg))")
  })

  test("power", () => {
    expect(show("declare hz (1 / s ^ 1)")).toBe(
      "(declare hz (composite (div 1 (pow s 1))))",
    )
  })

  test("composite with mul and div", () => {
    const src = "declare a base x\ndeclare b base y\ndeclare ab (a * b / 2)"
    expect(show(src)).toBe(
      [
        "(declare a (base x))",
        "(declare b (base y))",
        "(declare ab (composite (div (mul a b) 2)))",
      ].join("\n"),
    )
  })
  ...
  test("negative integer exponent in unit_expr (inside as conversion)", () => {
    ...
    const src = "declare m base length\ndeclare s base time\n9,8 (m / s ^ 2) g\ng as m * s ^ -2"
    const { program } = parse(src)
...
describe("program structure and recovery", () => {
  test("blank lines and comments are ignored between statements", () => {
    const src = ["# header", "", "declare kg base mass", "", "# done"].join("\n")
    expect(show(src)).toBe("(declare kg (base mass))")
  })

  test("error in one line does not stop later lines", () => {
    const src = ["bad +", "declare kg base mass"].join("\n")
    const { program, diagnostics } = parse(src)
    expect(diagnostics.length).toBeGreaterThan(0)
    expect(program.statements.length).toBe(1)
    expect(showStatement(program.statements[0]!)).toBe("(declare kg (base mass))")
  })
...
describe("budget.calc round-trip", () => {
  test("full file parses without diagnostics", async () => {
    ...
    // 7 unit declares + 3 variable_decl + 2 expr_assignment + 1 bare expression
